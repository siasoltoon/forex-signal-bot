...
async def signal_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Run the production analysis and register actionable signals for lifecycle monitoring."""
    ...
    try:
        ...
        await status_message.edit_text("❌ <b>تولید سیگنال انجام نشد.</b>\n\n" f"علت فنی: <code>{type(exc).__name__}</code>\n" "داده بازار یا یکی از مراحل تحلیل در دسترس نبود؛ بنابراین سیگنال ساختگی صادر نشد.", parse_mode="HTML")
