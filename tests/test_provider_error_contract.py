...
PROVIDERS = [
    ...
    (AlphaVantageProvider, "get_forex_intraday", "alphavantage"),
]

@pytest.mark.parametrize("provider_cls,client_method,provider_name", PROVIDERS, ids=["oanda", "finnhub", "alphavantage"])
@pytest.mark.asyncio
async def test_get_candles_maps_client_exception_to_application_error(provider_cls, client_method, provider_name):
    client = AsyncMock()
    ...
    assert error.details["symbol"] == ("EUR_USD" if provider_name == "oanda" else "EURUSD")
    assert error.details["limit"] == 10
    getattr(client, client_method).assert_awaited_once()

@pytest.mark.parametrize("provider_cls,client_method", [(p, m) for p, m, _ in PROVIDERS], ids=["oanda", "finnhub", "alphavantage"])
@pytest.mark.asyncio
...
@pytest.mark.parametrize("provider_cls,client_method", [(p, m) for p, m, _ in PROVIDERS], ids=["oanda", "finnhub", "alphavantage"])
@pytest.mark.asyncio
async def test_get_candles_does_not_call_client_for_invalid_request(provider_cls, client_method):
    client = AsyncMock()
    provider = provider_cls(client=client)
